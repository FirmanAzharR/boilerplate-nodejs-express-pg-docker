...
router.get("/user/current", verifyJwt, async (req, res, next) => {
  try {
...
router.patch("/user/update-password", verifyJwt,async (req, res, next) => {
  try {
...
router.patch("/user/update-image", verifyJwt, uploadImg, async (req, res, next) => {
  try {
    // let reqBody = req.body;
    const decodeToken = req.decodeToken;
